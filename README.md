...
<div class="header">
  <div class="header-left">
    <div class="logo"><i class="fas fa-home"></i> 房贷推演系统</div>
    <div class="header-sub">v10.0 · 逐年年终奖变动 · 支出变动 · 精准计算</div>
  </div>
  <div class="header-badges">
    <span class="badge badge-blue"><i class="fas fa-percentage"></i> 逐月DTI</span>
    <span class="badge badge-green"><i class="fas fa-chart-line"></i> 收入增长</span>
    <span class="badge badge-purple"><i class="fas fa-flask"></i> 压力测试</span>
    <span class="badge badge-bonus"><i class="fas fa-gift"></i> 逐年年终奖</span>
    <span class="badge badge-teal"><i class="fas fa-wallet"></i> 支出变动</span>
  </div>
</div>
...
<div class="layout">
  <div class="sidebar">
    <div class="sidebar-inner">
      <!-- 收支基础 -->
      <div>
        <div class="s-head"><i class="fas fa-calendar"></i> 收支基础</div>
        <div class="field-group">
          <div class="field"><label>开始月份</label><input type="month" id="startDate"></div>
          <div class="field">
            <label>贷款组合</label>
            <select id="loanType" onchange="toggleInputs()">
              <option value="combined">组合贷</option>
              <option value="commercial">仅商贷</option>
              <option value="fund">仅公积金</option>
            </select>
          </div>
        </div>
        <div class="field-group">
          <div class="field"><label>家庭月收入</label><div class="input-wrap"><input type="number" id="income" value="25000"><span class="input-unit">元</span></div></div>
          <div class="field"><label>固定月支出</label><div class="input-wrap"><input type="number" id="expenses" value="6000"><span class="input-unit">元</span></div></div>
        </div>
        <div class="field-group">
          <div class="field"><label>年收入增速</label><div class="input-wrap"><input type="number" id="incomeGrowth" value="0" step="0.1" min="0" max="20"><span class="input-unit">%</span></div></div>
          <div class="field"><label>支出年增速</label><div class="input-wrap"><input type="number" id="expGrowth" value="0" step="0.1" min="0" max="20"><span class="input-unit">%</span></div></div>
        </div>
      </div>

      <!-- 年终奖基础设置 -->
      <div class="loan-box bonus-box">
        <div class="s-head" style="color:var(--bonus)"><i class="fas fa-gift"></i> 年终奖默认设置</div>
        <div class="field-group">
          <div class="field"><label>首次发放年份</label><input type="number" id="bonusYear" placeholder="如2025"></div>
          <div class="field"><label>默认金额</label><div class="input-wrap"><input type="number" id="bonusAmt" value="0" step="0.1"><span class="input-unit">万</span></div></div>
        </div>
        <div class="field-group">
          <div class="field">
            <label>发放月份</label>
          ...
          <div class="field"><label>默认年增速</label><div class="input-wrap"><input type="number" id="bonusGrowth" value="0" step="0.1"><span class="input-unit">%</span></div></div>
        </div>
        <div style="margin-bottom:4px">
          <label style="margin-bottom:3px;display:block">默认用途策略</label>
          <div class="bonus-use-row">
            <div class="bonus-use-btn act-prepay" id="bonusUse-prepay" onclick="selBonusUse('prepay')">🚀 全额提前还</div>
            <div class="bonus-use-btn" id="bonusUse-save" onclick="selBonusUse('save')">💰 全额储蓄</div>
            <div class="bonus-use-btn" id="bonusUse-split" onclick="selBonusUse('split')">⚖️ 部分还款</div>
          </div>
        </div>
        <div id="bonusSplitGroup" style="display:none" class="field-group">
          <div class="field"><label>默认还款比例</label><div class="input-wrap"><input type="number" id="bonusSplitRatio" value="50" min="0" max="100"><span class="input-unit">%</span></div></div>
          <div class="field">
            <label>还款对象</label>
            <select id="bonusSplitTarget"><option value="com">商贷</option><option value="fund">公积金</option><option value="both">按比例</option></select>
          </div>
        </div>
        <div id="bonusPrepayGroup">
          <label style="margin-bottom:2px;display:block">默认还款对象</label>
          <select id="bonusPrepayTarget">
            <option value="com">商贷</option><option value="fund">公积金</option><option value="both">按比例</option>
          </select>
        </div>
      </div>
      ...
      <div id="box-com" class="loan-box com">
        <div class="s-head" style="color:var(--primary)"><i class="fas fa-building"></i> 商业贷款</div>
        <div class="field-group">
          <div class="field"><label>总额</label><div class="input-wrap"><input type="number" id="cAmt" value="100"><span class="input-unit">万</span></div></div>
          <div class="field"><label>利率</label><div class="input-wrap"><input type="number" id="cRate" value="3.2" step="0.01"><span class="input-unit">%</span></div></div>
        </div>
        <div class="field-group">
          <div class="field"><label>年限</label><div class="input-wrap"><input type="number" id="cYear" value="30"><span class="input-unit">年</span></div></div>
          <div class="field"><label>方式</label><select id="cMethod"><option value="1">等额本息</option><option value="2">等额本金</option></select></div>
        </div>
      </div>

      <!-- 公积金 -->
      <div id="box-fund" class="loan-box fund">
        <div class="s-head" style="color:var(--cyan)"><i class="fas fa-landmark"></i> 公积金贷款</div>
        <div class="field-group">
          <div class="field"><label>总额</label><div class="input-wrap"><input type="number" id="fAmt" value="60"><span class="input-unit">万</span></div></div>
          <div class="field"><label>利率</label><div class="input-wrap"><input type="number" id="fRate" value="2.85" step="0.01"><span class="input-unit">%</span></div></div>
        </div>
        <div class="field-group">
          <div class="field"><label>年限</label><div class="input-wrap"><input type="number" id="fYear" value="30"><span class="input-unit">年</span></div></div>
          <div class="field"><label>方式</label><select id="fMethod"><option value="1">等额本息</option><option value="2">等额本金</option></select></div>
        </div>
      </div>

      <!-- 公积金对冲 -->
      <div>
        <div class="s-head"><i class="fas fa-piggy-bank"></i> 公积金对冲</div>
        <div class="field-group">
          <div class="field"><label>账户余额</label><div class="input-wrap"><input type="number" id="pfBal" value="40000"><span class="input-unit">元</span></div></div>
          <div class="field"><label>月缴（双边）</label><div class="input-wrap"><input type="number" id="pfAdd" value="3000"><span class="input-unit">元</span></div></div>
        </div>
        <div class="field-group">
          <div class="field">
            <label>对冲策略</label>
            <select id="pfMode" onchange="togglePfDate()">
              <option value="month">每月抵扣</option>
              <option value="year">年度冲抵(1月)</option>
      ...
      <!-- 提前还款选项 -->
      <div>
        <div class="s-head"><i class="fas fa-sliders-h"></i> 提前还款方式</div>
        <div class="prepay-opts">
          <div class="prepay-opt active" id="opt-term" onclick="selPrepayOpt('term')">缩短年限</div>
          <div class="prepay-opt" id="opt-amt" onclick="selPrepayOpt('amt')">减少月供</div>
        </div>
      </div>

      <button class="calc-btn" onclick="doCalc()">
        <i class="fas fa-calculator"></i> 执行推演
      </button>
    </div>
  </div>
  ...
  <div class="main">
    <!-- KPI -->
    <div class="metrics">
      <div class="metric blue"><div class="metric-sub">总成本</div><div class="metric-val" id="kpi-int">0</div><div class="metric-lbl">总利息 (万元)</div></div>
      <div class="metric red"><div class="metric-sub">利息率</div><div class="metric-val" id="kpi-ratio">0%</div><div class="metric-lbl">利息/本金比</div></div>
      <div class="metric green"><div class="metric-sub">财富</div><div class="metric-val" id="kpi-net">0</div><div class="metric-lbl">期末净储蓄 (万元)</div></div>
      <div class="metric yellow"><div class="metric-sub">DTI</div><div class="metric-val" id="kpi-dti">0%</div><div class="metric-lbl">初始现金DTI</div></div>
      <div class="metric purple"><div class="metric-sub">里程碑</div><div class="metric-val" id="kpi-date" style="font-size:14px">--</div><div class="metric-lbl">预计上岸日</div></div>
    </div>
    ...
    <div class="charts-row">
      <div class="chart-card" style="flex:1.4">
        <div class="chart-card-head">
          <span>资产 vs 负债 赛跑</span>
          <div class="legend-dots">
            <div class="legend-dot"><div class="dot" style="background:var(--green)"></div>净储蓄</div>
            <div class="legend-dot"><div class="dot" style="background:var(--red)"></div>剩余债务</div>
            <div class="legend-dot"><div class="dot" style="background:var(--bonus)"></div>年终奖累计</div>
          </div>
        </div>
      ...
      <div class="chart-card" style="flex:1.4">
        <div class="chart-card-head">
          <span>逐月 DTI 走势</span>
          <div class="legend-dots">
            <div class="legend-dot"><div class="dot" style="background:var(--purple)"></div>现金DTI</div>
            <div class="legend-dot"><div class="dot" style="background:var(--yellow)"></div>总DTI</div>
          </div>
        </div>
        <div class="chart-card-body"><canvas id="chartDTI"></canvas></div>
      </div>
      <div class="chart-card" style="flex:1">
        <div class="chart-card-head"><span>本息结构（累计）</span></div>
    ...
    <div class="bottom">
        ...
        <div class="card-head">
          <div>
            ...
            <span style="font-size:var(--fs-xs);color:var(--text3)">
              ...
              <span style="color:var(--yellow)">警告</span>
              <span style="color:var(--red)">超标/负结余</span>
              <span style="color:var(--bonus)">年终奖</span>
            </span>
          </div>
          <div class="year-nav">
            ...
            <button class="btn-small btn-export" style="margin-left:5px" onclick="exportExcel()"><i class="fas fa-download"></i> 导出</button>
          </div>
        </div>
        <div class="col-toggle-bar">
          <span>列组：</span>
          <div class="ctog on" id="tog-base" onclick="toggleCols('base')">基础</div>
          <div class="ctog on" id="tog-dti" onclick="toggleCols('dti')">DTI</div>
          <div class="ctog on" id="tog-com" onclick="toggleCols('com')">商贷</div>
          <div class="ctog on" id="tog-fund" onclick="toggleCols('fund')">公贷</div>
          <div class="ctog on" id="tog-prepay" onclick="toggleCols('prepay')">提前还</div>
          <div class="ctog on" id="tog-bonus" onclick="toggleCols('bonus')">年终奖</div>
        </div>
      ...
      <div class="tools-card">
        <div class="tabs">
          <div class="tab active" id="tab-report" onclick="switchTab('report')"><i class="fas fa-brain"></i> 智能分析</div>
          <div class="tab" id="tab-pay" onclick="switchTab('pay')"><i class="fas fa-coins"></i> 提前还款</div>
          <div class="tab" id="tab-other" onclick="switchTab('other')"><i class="fas fa-sliders-h"></i> 变动</div>
        </div>
        ...
        <div id="panel-pay" class="tab-panel">
          <div class="tool-section">
            <div class="tool-section-head"><i class="fas fa-magic"></i> 批量定期还款</div>
            <label style="margin-bottom:5px;display:block">还款对象</label>
            <div class="prepay-target-row" id="regTargetRow">
              <div class="prepay-target-btn act-com" id="regTgt-com" onclick="selRegTarget('com')"><i class="fas fa-building"></i> 商贷</div>
              <div class="prepay-target-btn" id="regTgt-fund" onclick="selRegTarget('fund')"><i class="fas fa-landmark"></i> 公积金</div>
              <div class="prepay-target-btn" id="regTgt-both" onclick="selRegTarget('both')"><i class="fas fa-balance-scale"></i> 按比例</div>
            </div>
            <div class="field-group">
              <div class="field"><label>首次时间</label><input type="month" id="regStart"></div>
              <div class="field"><label>间隔月</label><input type="number" id="regInterval" value="12" min="1"></div>
            </div>
            <div class="field-group">
              <div class="field"><label>每次金额(万)</label><input type="number" id="regAmt" value="5" step="0.1"></div>
              <div class="field"><label>共几次</label><input type="number" id="regTimes" value="5" min="1"></div>
            </div>
            <button class="btn-small btn-add" onclick="genRegular()" style="width:100%">
              <i class="fas fa-bolt"></i> 一键生成定期还款计划
            </button>
          </div>
          <div class="tool-section">
            <div class="tool-section-head"><i class="fas fa-plus-circle"></i> 单笔添加</div>
            <label style="margin-bottom:5px;display:block">还款对象</label>
            <div class="prepay-target-row" id="singleTargetRow">
              <div class="prepay-target-btn act-com" id="singleTgt-com" onclick="selSingleTarget('com')"><i class="fas fa-building"></i> 商贷</div>
              <div class="prepay-target-btn" id="singleTgt-fund" onclick="selSingleTarget('fund')"><i class="fas fa-landmark"></i> 公积金</div>
              <div class="prepay-target-btn" id="singleTgt-both" onclick="selSingleTarget('both')"><i class="fas fa-balance-scale"></i> 按比例</div>
            </div>
            <div class="field-group">
              <div class="field"><label>时间</label><input type="month" id="pyDate"></div>
              <div class="field"><label>金额(万)</label><input type="number" id="pyAmt" step="0.1"></div>
            </div>
            <div style="display:flex;gap:5px;align-items:center">
              <span style="font-size:var(--fs-sm);color:var(--text3)">已添加 <span id="payCount" style="color:var(--orange);font-weight:700">0</span> 笔</span>
              <button class="btn-small btn-add" onclick="addPay()" style="margin-left:auto"><i class="fas fa-plus"></i> 添加</button>
            </div>
          </div>
        ...
        <div id="panel-other" class="tab-panel">
          ...
          <!-- 逐年年终奖个性化（事件式）-->
          <div class="tool-section">
            <div class="tool-section-head"><i class="fas fa-gift"></i> 逐年年终奖个性化 <span style="color:var(--bonus);font-size:10px">●</span></div>

            <div class="yb-input-grid">
              <div class="field"><label>年份</label><input type="number" id="ybYear" placeholder="如2026" min="2026" max="2060"></div>
              <div class="field"><label>金额(万，留空=默认)</label><input type="number" id="ybAmt" step="0.1" placeholder="留空用默认"></div>
            </div>
            <div style="margin-bottom:4px">
              <div class="bonus-use-row">
                <div class="bonus-use-btn" id="ybUse-default" onclick="selYbUse('default')">默认</div>
                <div class="bonus-use-btn" id="ybUse-prepay" onclick="selYbUse('prepay')">🚀 全还</div>
                <div class="bonus-use-btn" id="ybUse-save" onclick="selYbUse('save')">💰 全存</div>
                <div class="bonus-use-btn" id="ybUse-split" onclick="selYbUse('split')">⚖️ 分配</div>
              </div>
            </div>
            <div id="ybSplitRow" style="display:none" class="field-group">
              <div class="field"><label>还款比例</label><div class="input-wrap"><input type="number" id="ybSplitRatio" value="50" min="0" max="100"><span class="input-unit">%</span></div></div>
              <div class="field"><label>还款对象</label><select id="ybSplitTgt"><option value="com">商贷</option><option value="fund">公积金</option><option value="both">按比例</option></select></div>
            </div>
            <div id="ybPrepayRow" class="field" style="display:none;margin-bottom:4px">
              <label>还款对象</label>
              <select id="ybPrepayTgt"><option value="com">商贷</option><option value="fund">公积金</option><option value="both">按比例</option></select>
            </div>
            <button class="btn-small btn-bonus" style="width:100%" onclick="addYbEvent()">
              <i class="fas fa-plus"></i> 添加该年份个性化设置
            </button>
          ...
          <!-- 利率变动 -->
          <div class="tool-section">
            <div class="tool-section-head"><i class="fas fa-percentage"></i> 利率变动<span style="color:var(--primary);font-size:10px">●</span></div>
            <div class="rate-type-row">
              <div class="rate-type-btn active" id="rateTypeCom" onclick="selRateType('com')"><i class="fas fa-building"></i> 商贷</div>
              <div class="rate-type-btn" id="rateTypeFund" onclick="selRateType('fund')"><i class="fas fa-landmark"></i> 公贷</div>
            </div>
            <div class="field-group">
              <div class="field"><label>生效时间</label><input type="month" id="rtDate"></div>
              <div class="field"><label>新利率(%)</label><input type="number" id="rtVal" step="0.01"></div>
              <button class="btn-small btn-add" onclick="addEv('rate')" style="align-self:flex-end"><i class="fas fa-plus"></i>添加</button>
            </div>
          ...
          <!-- 固定支出变动 -->
          <div class="tool-section">
            <div class="tool-section-head"><i class="fas fa-wallet"></i> 固定支出变动<span style="color:var(--primary);font-size:10px">●</span></div>
            <div style="font-size:var(--fs-xs);color:var(--text3);margin-bottom:5px">在特定时间点修改家庭月固定支出</div>
            <div class="field-group">
              <div class="field"><label>生效时间</label><input type="month" id="expChDate"></div>
              <div class="field"><label>新月支出(元)</label><input type="number" id="expChVal"></div>
              <button class="btn-small btn-add" onclick="addEv('exp')" style="align-self:flex-end"><i class="fas fa-plus"></i>添加</button>
            </div>
          ...
          <!-- 公积金月缴变动 -->
          <div class="tool-section">
            <div class="tool-section-head"><i class="fas fa-piggy-bank"></i> 公积金月缴变动<span style="color:var(00ffss);font-size:10px">●</span></div>
            <div class="field-group">
              <div class="field"><label>生效时间</label><input type="month" id="pfChDate"></div>
              <div class="field"><label>新月缴(元)</label><input type="number" id="pfChVal"></div>
              <button class="btn-small btn-add" onclick="addEv('pf')" style="align-self:flex-end"><i class="fas fa-plus"></i>添加</button>
            </div>
          ...
          <!-- 收入阶段性跳涨 -->
          <div class="tool-section">
            <div class="tool-section-head"><i class="fas fa-user-tie"></i> 收入阶段性跳涨<span style="color:var(--bonus);font-size:10px">●</span></div>
            <div style="font-size:var(--fs-xs);color:var(--text3);margin-bottom:6px">在特定时间点设置新月收入（覆盖增速公式）</div>
            <div class="field-group">
              <div class="field"><label>时间</label><input type="month" id="incDate"></div>
              <div class="field"><label>新月收入(元)</label><input type="number" id="incVal"></div>
              <button class="btn-small btn-add" onclick="addEv('inc')" style="align-self:flex-end"><i class="fas fa-plus"></i>添加</button>
            </div>
...
// ══════════════════════════════════════════════════════
const COL_DEFS = [
  {key:'month',      group:'base',   label:'月份'},
  {key:'inc',        group:'base',   label:'收入'},
  {key:'exp',        group:'base',   label:'固支'},
  {key:'totalPay',   group:'base',   label:'月供总'},
  {key:'cashPay',    group:'base',   label:'现金供'},
  {key:'cover',      group:'base',   label:'公积金冲'},
  {key:'savingFlow', group:'base',   label:'月净结余'},
  {key:'cumSaving',  group:'base',   label:'累计净储蓄'},
  {key:'dtiCash',    group:'dti',    label:'现金DTI', thCls:'th-dti', grpSep:true},
  {key:'dtiTotal',   group:'dti',    label:'总DTI',   thCls:'th-dti'},
  {key:'comPay',     group:'com',    label:'商月供',  grpSep:true},
  {key:'comPrin',    group:'com',    label:'商本'},
  {key:'comInt',     group:'com',    label:'商息'},
  {key:'remC',       group:'com',    label:'商余额'},
  {key:'fundPay',    group:'fund',   label:'公月供',  grpSep:true},
  {key:'fundPrin',   group:'fund',   label:'公本'},
  {key:'fundInt',    group:'fund',   label:'公息'},
  {key:'remF',       group:'fund',   label:'公余额'},
  {key:'extraCom',   group:'prepay', label:'商提还',  grpSep:true},
  {key:'extraFund',  group:'prepay', label:'公提还'},
  {key:'bonus',      group:'bonus',  label:'年终奖',  grpSep:true, thCls:'th-bonus'},
  {key:'bonusPrepay',group:'bonus',  label:'奖-还款'},
  {key:'bonusSave',  group:'bonus',  label:'奖-储蓄'},
  {key:'notes',      group:'base',   label:'备注'},
];
...
function addYbEvent(){
  ...
  if(!yr||yr<2025||yr>2080){ alert('请填写有效年份'); return; }
...
function renderYbList(){
  ...
  const useLabels={default:'默认',prepay:'全还',save:'全存',split:'分配'};
  const useCls={default:'',prepay:'prepay',save:'save',split:'split'};
  el.innerHTML=events.yb.map((e,i)=>{
    const amtStr=e.amt!==null?`<span class="bonus-ev-amt">${e.amt}万</span>`:'<span style="color:var(--text3);font-size:9px">默认金额</span>';
    const useStr=e.use!=='default'?`<span class="bonus-ev-use ${useCls[e.use]}">${useLabels[e.use]}</span>`:'<span class="bonus-ev-use" style="background:var(--surface3);color:var(--text3)">默认策略</span>';
    let tgtStr='';
    if(e.use==='split') tgtStr=`<span class="bonus-ev-tgt">${e.splitRatio||50}%→${e.splitTgt==='com'?'商':e.splitTgt==='fund'?'公':'比'}</span>`;
    else if(e.use==='prepay') tgtStr=`<span class="bonus-ev-tgt">→${e.prepayTgt==='com'?'商':e.prepayTgt==='fund'?'公':'比'}</span>`;
    return `<div class="bonus-event-item"><span class="bonus-ev-year">${e.year}年</span>${amtStr}${useStr}${tgtStr}<span class="del-btn" onclick="delYbEvent(${i})"><i class="fas fa-times"></i></span></div>`;
  }).join('');
}
...
function genRegular(){
  ...
  const tgt=regTarget;
  if(!start||amt<=0||times<=0){alert('请填写完整参数');return;}
...
function renderList(type){
  if(type==='pay'){
    ...
    el.innerHTML=sorted.map((e,i)=>{
      ...
      const tgtLabel=e.target==='com'?`<span style="font-size:var(--fs-xs);color:var(--primary);font-weight:700">[商]</span>`:e.target==='fund'?`<span style="font-size:var(--fs-xs);color:var(--cyan);font-weight:700">[公]</span>`:`<span style="font-size:var(--fs-xs);color:var(--green);font-weight:700">[比]</span>`;
      const tgtColor=e.target==='com'?'var(--primary)':e.target==='fund'?'var(--cyan)':'var(--green)';
      return `<div class="event-item"><span class="event-date">${e.date} ${tgtLabel}</span><span class="event-val" style="color:${tgtColor}">${(e.val/10000).toFixed(1)}万</span><span class="del-btn" onclick="delEv('pay',${origIdx})"><i class="fas fa-times"></i></span></div>`;
    }).join('');
  ...
  if(!el) return;
  const units={rate:'%',pf:'元',inc:'元',exp:'元'};
  ...
  el.innerHTML=sorted.map((e)=>{
    ...
    const tag=type==='rate'?`<span style="font-size:var(--fs-xs);color:${e.target==='com'?'var(--primary)':'var(--cyan)'}">[${e.target==='com'?'商':'公'}]</span> `:'';
    ...
    if(remMonths>0){
      if(mode==='term'){
        ...
        if(l.m==='1'){
          ...
          // 等额本金：月还本金不变，自然缩短
          l.basePrin=l.initBasePrin; // 保持原还本金
...
function doCalc(){
  ...
  const bonusAmtBase=gN('bonusAmt')*10000; // 元
  ...
  let stats={
    ...
    firstComInt:0, firstFundInt:0,  // 用于压力测试
    firstCashPay:0,
  };
  ...
  for(let i=1;i<=600&&active;i++){
    ...
    loans.forEach(l=>{
      ...
      let prin,pay;
      if(l.m==='1'){
        // 等额本息：月供固定，最后一期可能略有差异
        pay=l.payExact;
        prin=pay-int;
        if(prin<0) prin=0; // 利率变动极端情况
        if(prin>l.curP){ prin=l.curP; pay=prin+int; } // 最后一期
      } else {
        // 等额本金：每月还固定本金
    ...
    if(pfMode!=='none'&&ds>=pfStart){
      if(pfMode==='month'){
        ...
        // 年度冲抵：用当年积累的公积金冲抵当月月供（象征性，实际应按余额冲）
        const maxCover=Math.min(pfBal,mDue);
        if(maxCover>1000){ cover=maxCover; pfBal-=cover; yfNote='年冲'; }
      }
    }
    const cashPay=Math.max(0,mDue-cover); // 需要现金支付的月供

    // ── DTI 计算 ──
    // 现金DTI = 现金月供 / 月收入（反映实际现金压力）
    // 总DTI = (现金月供 + 固定支出) / 月收入（反映总负担）
    ...
    //
    // 现金流入：月收入 + 年终奖中"储蓄部分"
    //   （年终奖"还款部分"属于现金流出，不进储蓄）
    // 现金流出：固定支出 + 现金月供 + 手动提前还款
    //   （年终奖还款已在 bonusPrepayThis 中，但该金额
    //    来自年终奖收入，净效果 = bonusSaveThis 进储蓄）
    //
    // savingFlow = 收入 + 年终奖 - 固支 - 现金月供
    //              - 年终奖还款部分 - 手动提还
    //           = 收入 + bonusSaveThis - 固支 - 现金月供 - 手动提还
    //   （两种写法等价）
    // ══════════════════════════════════════════
    ...
    // ── 备注标签 ──
    let notes=[];
    if(bonusThisMonth>0)  notes.push({t:`奖${(bonusThisMonth/10000).toFixed(1)}万`,cls:'ntag-bonus'});
    if(extraCom>0)        notes.push({t:'商提',cls:'ntag-prepay-com'});
    if(extraFund>0)       notes.push({t:'公提',cls:'ntag-prepay-fund'});
    if(bonusPrepayThis>0) notes.push({t:'奖还',cls:'ntag-bonus'});
    if(yfNote)            notes.push({t:'年冲',cls:'ntag-year'});
    if(rateNote)          notes.push({t:'利率变',cls:'ntag-rate'});
    if(evPf)              notes.push({t:'公积金调',cls:'ntag-pf'});
    if(evInc)             notes.push({t:'收入变',cls:'ntag-pf'});
    if(evExp)             notes.push({t:'支出变',cls:'ntag-exp'});
...
function renderUI(stats, initDebt, baselineInterest){
  ...
  document.getElementById('kpi-date').textContent=stats.crossDate||'未上岸';
  ...
  // ── 1. DTI 报告（以现金DTI分段，总DTI辅助展示）──
  html+=`<div class="report-block">
  <div class="rb-head" style="color:var(--purple)"><i class="fas fa-percentage"></i> DTI全周期精确追踪（共${total}个月）</div>
  <div class="rb-body">
    <div style="font-size:var(--fs-xs);color:var(--text3);margin-bottom:4px">现金DTI = 现金月供 / 月收入（进度条按现金DTI分段）</div>
    ...
    <div style="display:flex;gap:10px;font-size:var(--fs-xs);color:var(--text3);margin-bottom:8px;flex-wrap:wrap">
      <span style="color:var(--green)">● 安全(&lt;40%) ${safeMo}月</span>
      <span style="color:var(--yellow)">● 警告(40-50%) ${warnMo}月</span>
      <span style="color:var(--red)">● 超标(≥50%) ${overMo}月</span>
    </div>
    <div class="stat-grid-4">
      <div class="stat-cell"><div class="stat-cell-val" style="color:${dtiColor(firstRow.dtiCash)}">${firstRow.dtiCash.toFixed(1)}%</div><div class="stat-cell-lbl">首月现金DTI</div><div class="stat-cell-sub">${firstRow.d}</div></div>
      <div class="stat-cell"><div class="stat-cell-val" style="color:${dtiColor(firstRow.dtiTotal)}">${firstRow.dtiTotal.toFixed(1)}%</div><div class="stat-cell-lbl">首月总DTI</div><div class="stat-cell-sub">含固支</div></div>
      <div class="stat-cell"><div class="stat-cell-val" style="color:${dtiColor(avgCashDTI)}">${avgCashDTI.toFixed(1)}%</div><div class="stat-cell-lbl">均值现金DTI</div></div>
      <div class="stat-cell"><div class="stat-cell-val" style="color:${dtiColor(maxCashDTI)}">${maxCashDTI.toFixed(1)}%</div><div class="stat-cell-lbl">峰值现金DTI</div><div class="stat-cell-sub">${stats.dtiCashArr[maxCashDTIIdx]?.date||''}</div></div>
    </div>
    ${bi(firstRow.dtiCash>=50,'bad',`首月现金DTI ${firstRow.dtiCash.toFixed(1)}%，已超50%压力线，银行审批存风险`)}
    ${bi(firstRow.dtiCash>=40&&firstRow.dtiCash<50,'warn',`首月现金DTI ${firstRow.dtiCash.toFixed(1)}%，处于40-50%警戒区间`)}
    ${bi(firstRow.dtiCash<40,'ok',`首月现金DTI ${firstRow.dtiCash.toFixed(1)}%，符合银行主流审批要求（<40%）`)}
    ${bi(firstRow.dtiTotal>=60,'bad',`首月总DTI（含固支）${firstRow.dtiTotal.toFixed(1)}%，超过60%，整体财务压力较大`)}
    ${bi(firstRow.dtiTotal>=50&&firstRow.dtiTotal<60,'warn',`首月总DTI（含固支）${firstRow.dtiTotal.toFixed(1)}%，处于警戒区间`)}
    ${bi(overMo>0,'warn',`共 ${overMo} 个月现金DTI≥50%，建议保留6个月以上应急储备`)}
    ${bi(overMo===0&&avgCashDTI<35,'ok','全周期现金DTI均在安全区间，财务稳健')}
  </div></div>`;

  // ── 2. 年终奖报告 ──
  if(stats.bonusCount>0){
    html+=`<div class="report-block">
  <div class="rb-head" style="color:var(--bonus)"><i class="fas fa-gift"></i> 年终奖规划分析（共${stats.bonusCount}次）</div>
  <div class="rb-body">
    <div class="stat-grid-4">
      <div class="stat-cell"><div class="stat-cell-val" style="color:var(--bonus)">${(stats.bonusTotalAmt/10000).toFixed(1)}万</div><div class="stat-cell-lbl">累计年终奖</div></div>
      <div class="stat-cell"><div class="stat-cell-val" style="color:var(--orange)">${(stats.bonusPrepayAmt/10000).toFixed(1)}万</div><div class="stat-cell-lbl">用于还款</div></div>
      <div class="stat-cell"><div class="stat-cell-val" style="color:var(--green)">${(stats.bonusSaveAmt/10000).toFixed(1)}万</div><div class="stat-cell-lbl">用于储蓄</div></div>
      <div class="stat-cell"><div class="stat-cell-val" style="color:var(--red)">${(interestSaved/10000).toFixed(2)}万</div><div class="stat-cell-lbl">所有提前还款节省利息</div></div>
    </div>
    <div class="formula">年终奖还款额 ${(stats.bonusPrepayAmt/10000).toFixed(1)}万 + 手动提前还款 ${(stats.prepayManualTotal/10000).toFixed(1)}万 = 合计 ${(stats.prepayTotal/10000).toFixed(1)}万</div>
    ${bi(stats.bonusPrepayAmt>0&&interestSaved>0,'ok',`提前还款共节省利息 ${(interestSaved/10000).toFixed(2)} 万元（含年终奖+手动还款）`)}
    ${bi(stats.bonusSaveAmt>stats.bonusPrepayAmt,'info','年终奖以储蓄为主，流动性充裕')}
    ${bi(stats.bonusPrepayAmt>stats.bonusTotalAmt*0.7,'info',`超过70%年终奖用于提前还款，可显著压缩还贷周期`)}
  </div></div>`;
  }

  // ── 3. 压力测试（计算更精确）──
  // 首月现金月供（未经公积金冲抵前）= firstRow.totalPay
  // 利率+50BP：新月供 = PMT(r+0.5/1200, n, P) 近似 firstRow.totalPay*(1+0.005/(12*r))
  // 更准确：直接重新算
  ...
  html+=`<div class="report-block">
  <div class="rb-head" style="color:var(--red)"><i class="fas fa-flask"></i> 压力情景模拟（基于首月数据）</div>
  <div class="rb-body">
    <div class="stress-grid">`;
  [
    {label:'收入-20%，现金DTI',val:dtiStress80.toFixed(1)+'%',ok:dtiStress80<50},
    {label:'固支+20%，总DTI', val:expUp20.toFixed(1)+'%',ok:expUp20<60},
    {label:'利率+50BP，月供DTI',val:dtiAt50bp.toFixed(1)+'%',ok:dtiAt50bp<50},
    {label:'利率+100BP，月供DTI',val:dtiAt100bp.toFixed(1)+'%',ok:dtiAt100bp<50},
    {label:'当前总DTI（含固支）',val:firstRow.dtiTotal.toFixed(1)+'%',ok:firstRow.dtiTotal<50},
    {label:'失业6月现金流出（月供×6）',val:fmtAlways(firstRow.cashPay*6)+'元',ok:true},
  ].forEach(s=>{
    const c=s.ok?'var(--green)':'var(--red)';
    html+=`<div class="stress-cell"><div class="stress-val" style="color:${c}">${s.ok?'✓':'⚠'} ${s.val}</div><div class="stress-lbl">${s.label}</div></div>`;
  });
  html+=`</div>
    ${bi(!stats.firstComRate&&!stats.firstFundRate,'info','无贷款信息，压力测试仅供参考')}
  </div></div>`;

  // ── 4. 里程碑 ──
  html+=`<div class="report-block">
  <div class="rb-head" style="color:var(--yellow)"><i class="fas fa-flag-checkered"></i> 还贷里程碑</div>
  <div class="rb-body milestone-list">`;
  [{label:'🏦 还清一半债务',date:stats.halfDebtPaid||'—',ok:!!stats.halfDebtPaid},
   {label:'⚡ 月还本金>月还利息（黄金交叉）',date:stats.goldCross||'—',ok:!!stats.goldCross},
   {label:'🎯 剩余债务仅25%',date:stats.quarterLeft||'—',ok:!!stats.quarterLeft},
   {label:'🏁 净储蓄首次超过剩余债务（上岸）',date:stats.crossDate||'—',ok:!!stats.crossDate},
   {label:'🎉 贷款全部还清',date:stats.loanEndDate||'—',ok:!!stats.loanEndDate},
  ].forEach(ms=>{
    const c=ms.ok?'var(--green)':'var(--text3)';
    html+=`<div class="milestone"><span class="ms-label">${ms.label}</span><span style="display:flex;align-items:center;gap:7px"><span class="ms-date" style="color:${c}">${ms.date}</span><span style="color:${c}">${ms.ok?'✓':'○'}</span></span></div>`;
  });
  html+=`</div></div>`;
  ...
  html+=`<div class="report-block">
  <div class="rb-head" style="color:var(--cyan)"><i class="fas fa-heartbeat"></i> 现金流健康度</div>
  <div class="rb-body">
    <div class="stat-grid-4">
      <div class="stat-cell"><div class="stat-cell-val" style="color:${avgSavingFlow>0?'var(--green)':'var(--red)'}">${(avgSavingFlow/10000).toFixed(2)}万</div><div class="stat-cell-lbl">月均净结余</div></div>
      <div class="stat-cell"><div class="stat-cell-val" style="color:${stats.minSaving>=0?'var(--yellow)':'var(--red)'}">${(stats.minSaving/10000).toFixed(2)}万</div><div class="stat-cell-lbl">最低月结余</div><div class="stat-cell-sub">${stats.minSavingDate}</div></div>
      <div class="stat-cell"><div class="stat-cell-val" style="color:${stats.riskMonths>0?'var(--red)':'var(--green)'}">${stats.riskMonths}</div><div class="stat-cell-lbl">负结余月数</div></div>
      <div class="stat-cell"><div class="stat-cell-val" style="color:var(--cyan)">${(finalCumSaving/10000).toFixed(1)}万</div><div class="stat-cell-lbl">期末净储蓄</div></div>
    </div>
    <div class="formula" style="font-size:9px">月净结余 = 月收入 + 年终奖 - 固定支出 - 现金月供 - 年终奖还款额 - 手动提前还款</div>
    ${bi(stats.riskMonths>0,'bad',`共 ${stats.riskMonths} 个月月净结余为负，最大缺口 ${fmtAlways(Math.abs(stats.maxDeficit))} 元（${stats.maxDeficitDate}），需提前备足现金`)}
    ${bi(stats.riskMonths===0&&avgSavingFlow>5000,'ok','全周期月净结余均为正，现金流健康')}
    ${bi(stats.riskMonths===0&&avgSavingFlow<=5000&&avgSavingFlow>0,'warn','月均结余偏低，建议维持至少6个月月供的应急储备')}
    ${bi(finalCumSaving<0,'bad',`期末净储蓄为负 ${(Math.abs(finalCumSaving)/10000).toFixed(1)} 万元，整体入不敷出`)}
    ${bi(finalCumSaving>=0&&finalCumSaving<initDebt*0.3,'warn',`期末净储蓄仅 ${(finalCumSaving/10000).toFixed(1)} 万元，占贷款本金 ${(finalCumSaving/initDebt*100).toFixed(0)}%`)}
    ${bi(finalCumSaving>=initDebt*0.3,'ok',`期末净储蓄 ${(finalCumSaving/10000).toFixed(1)} 万元，财富积累状况良好`)}
  </div></div>`;

  // ── 6. 提前还款效益（仅在有提前还款时显示）──
  if(stats.prepayTotal>0){
    html+=`<div class="report-block">
  <div class="rb-head" style="color:var(--orange)"><i class="fas fa-bolt"></i> 提前还款效益核算</div>
  <div class="rb-body">
    <div class="stat-grid-3">
      <div class="stat-cell"><div class="stat-cell-val" style="color:var(--orange)">${(stats.prepayTotal/10000).toFixed(1)}万</div><div class="stat-cell-lbl">累计提前还款</div></div>
      <div class="stat-cell"><div class="stat-cell-val" style="color:var(--green)">${(interestSaved/10000).toFixed(2)}万</div><div class="stat-cell-lbl">节省利息（对比不还）</div></div>
      <div class="stat-cell"><div class="stat-cell-val" style="color:var(--cyan)">${stats.prepayTotal>0?(interestSaved/stats.prepayTotal*100).toFixed(1):0}%</div><div class="stat-cell-lbl">每元提还节省利息</div></div>
    </div>
    <div class="stat-grid" style="margin-top:5px">
      <div class="stat-cell"><div class="stat-cell-val" style="color:var(--primary)">${(stats.prepayComTotal/10000).toFixed(1)}万</div><div class="stat-cell-lbl">→商贷部分</div></div>
      <div class="stat-cell"><div class="stat-cell-val" style="color:var(--cyan)">${(stats.prepayFundTotal/10000).toFixed(1)}万</div><div class="stat-cell-lbl">→公积金部分</div></div>
    </div>
    ${bi(interestSaved>stats.prepayTotal*0.3,'ok',`节省利息/还款额 = ${(interestSaved/Math.max(1,stats.prepayTotal)*100).toFixed(1)}%，提前还款效益显著`)}
    ${bi(interestSaved>0&&interestSaved<=stats.prepayTotal*0.3,'info',`节省利息/还款额 = ${(interestSaved/Math.max(1,stats.prepayTotal)*100).toFixed(1)}%，提前还款有一定效益`)}
  </div></div>`;
  }
...
function renderCharts(stats){
  ...
  const raceDS=[
    {label:'剩余债务',data:yrs.map(y=>+(byYear[y].rem/10000).toFixed(1)),borderColor:'#e05252',borderWidth:2,pointRadius:0,fill:false,tension:.3},
    {label:'净储蓄',  data:yrs.map(y=>+(Math.max(0,byYear[y].cumSaving)/10000).toFixed(1)),borderColor:'#38a169',borderWidth:2,pointRadius:0,fill:true,backgroundColor:'rgba(56,161,105,.08)',tension:.3},
  ];
  if(hasBonusData) raceDS.push({label:'年终奖累计',data:bonusData,borderColor:'#9b4dca',borderWidth:1.5,borderDash:[4,3],pointRadius:0,fill:false,tension:.3});
  chartRace=new Chart(ctxR,{type:'line',data:{labels:yrs,datasets:raceDS},options:{responsive:true,maintainAspectRatio:false,interaction:{mode:'index',intersect:false},plugins:{legend:{display:false},tooltip:{backgroundColor:'#fff',borderColor:'#d8e1ec',borderWidth:1,titleColor:'#2d3748',bodyColor:'#4a5568',callbacks:{label:ctx=>`${ctx.dataset.label}: ${ctx.raw.toFixed(1)}万`}}},scales:{x:{grid:{color:'rgba(216,225,236,.5)'},ticks:{color:'#8a9ab5',font:{size:10}}},y:{grid:{color:'rgba(216,225,236,.5)'},ticks:{color:'#8a9ab5',font:{size:10},callback:v=>v+'万'}}}}});
  ...
  chartDTI=new Chart(ctxD,{type:'line',data:{labels:sampled.map(r=>r.d),datasets:[
    {label:'现金DTI',data:sampled.map(r=>+r.dtiCash.toFixed(1)),borderColor:'#7c5cbf',borderWidth:1.5,pointRadius:0,fill:false,tension:.3},
    {label:'总DTI',  data:sampled.map(r=>+r.dtiTotal.toFixed(1)),borderColor:'#d97706',borderWidth:1.5,pointRadius:0,fill:false,tension:.3},
    {label:'50%线',  data:sampled.map(()=>50),borderColor:'rgba(224,82,82,.5)',borderWidth:1,borderDash:[4,4],pointRadius:0,fill:false},
    {label:'40%线',  data:sampled.map(()=>40),borderColor:'rgba(217,119,6,.3)',borderWidth:1,borderDash:[2,4],pointRadius:0,fill:false},
  ]},options:{responsive:true,maintainAspectRatio:false,interaction:{mode:'index',intersect:false},plugins:{legend:{display:false},tooltip:{backgroundColor:'#fff',borderColor:'#d8e1ec',borderWidth:1,titleColor:'#2d3748',bodyColor:'#4a5568',callbacks:{label:ctx=>{if(ctx.dataset.label==='50%线'||ctx.dataset.label==='40%线') return null;return `${ctx.dataset.label}: ${ctx.raw}%`;}}}},scales:{x:{grid:{color:'rgba(216,225,236,.5)'},ticks:{color:'#8a9ab5',font:{size:10},maxTicksLimit:8}},y:{min:0,max:maxDTI,grid:{color:'rgba(216,225,236,.5)'},ticks:{color:'#8a9ab5',font:{size:10},callback:v=>v+'%'}}}}});

  // 饼图：本金+利息
  const ctxP=document.getElementById('chartPie').getContext('2d');
  if(chartPie) chartPie.destroy();
  chartPie=new Chart(ctxP,{type:'doughnut',data:{labels:['商贷本金','公贷本金','商贷利息','公贷利息'],datasets:[{data:[
  ...
  fullData.filter(r=>r.y===curViewYear).forEach(r=>{
    ...
    cols.forEach((c,ci)=>{
      ...
      switch(c.key){
        ...
        case 'cover':
          td.textContent=r.cover>0?fmtAlways(r.cover):'—';
        ...
        case 'comPay':
          td.textContent=r.comPay>0?fmtAlways(r.comPay):'—';
          td.style.color=r.comPay>0?'var(--primary)':'var(--text3)'; break;
        case 'comPrin':
          td.textContent=r.comPrin>0?fmtAlways(r.comPrin):'—';
          if(!r.comPrin) td.style.color='var(--text3)'; break;
        case 'comInt':
          td.textContent=r.comInt>0?fmtAlways(r.comInt):'—';
        ...
        case 'fundPay':
          td.textContent=r.fundPay>0?fmtAlways(r.fundPay):'—';
          td.style.color=r.fundPay>0?'var(--cyan)':'var(--text3)'; break;
        case 'fundPrin':
          td.textContent=r.fundPrin>0?fmtAlways(r.fundPrin):'—';
          if(!r.fundPrin) td.style.color='var(--text3)'; break;
        case 'fundInt':
          td.textContent=r.fundInt>0?fmtAlways(r.fundInt):'—';
...
function exportExcel(){
  ...
  const ws=XLSX.utils.json_to_sheet(fullData.map(r=>({
    '月份':r.d,'月收入':r.inc,'固定支出':r.exp,
    '年终奖':r.bonus||'','年终奖-还款':r.bonusPrepay||'','年终奖-储蓄':r.bonusSave||'',
    '月供总额':r.totalPay,'公积金冲抵':r.cover||'','现金月供':r.cashPay,
    '月净结余':r.savingFlow,'累计净储蓄':r.cumSaving,
    '现金DTI%':+r.dtiCash.toFixed(2),'总DTI%':+r.dtiTotal.toFixed(2),
    '商贷月供':r.comPay||'','商贷本金':r.comPrin||'','商贷利息':r.comInt||'','商贷余额':r.remC,
    '公贷月供':r.fundPay||'','公贷本金':r.fundPrin||'','公贷利息':r.fundInt||'','公贷余额':r.remF,
    '商贷手动提还':r.extraCom||'','公贷手动提还':r.extraFund||'',
  })));
  const wb=XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb,ws,'推演明细');
  XLSX.writeFile(wb,`房贷推演v10_${new Date().toISOString().slice(0,10)}.xlsx`);
}
